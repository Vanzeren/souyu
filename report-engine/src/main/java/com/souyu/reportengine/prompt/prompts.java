package com.souyu.reportengine.prompt;
...
public class prompts {
    ...
    // 模板选择输出Schema
    public static final String OUTPUT_SCHEMA_TEMPLATE_SELECTION = """
            {
              "type": "object",
              "properties": {
                "template_name": {
    ...
    // HTML报告生成输入Schema
    public static final String INPUT_SCHEMA_HTML_GENERATION = """
            {
              "type": "object",
              "properties": {
                "query": {
    ...
    public static final String CHAPTER_GENERATION_INPUT_SCHEMA = """
            {
              "type": "object",
              "properties": {
                "section": {
    ...
    public static final String DOCUMENT_LAYOUT_OUTPUT_SCHEMA = """
            {
              "type": "object",
              "properties": {
                "title": {
    ...
    public static final String WORD_BUDGET_OUTPUT_SCHEMA = """
            {
              "type": "object",
              "properties": {
                "totalWords": {
    ...
    public static final String SYSTEM_PROMPT_TEMPLATE_SELECTION = """
            ...
            **重要的输出格式要求：**
            ...
            """.formatted(OUTPUT_SCHEMA_TEMPLATE_SELECTION);
    ...
    public static final String SYSTEM_PROMPT_HTML_GENERATION = """
            ...
            **重要：直接返回完整的HTML代码，不要包含任何解释、说明或其他文本。只返回HTML代码本身。**
            """.formatted(INPUT_SCHEMA_HTML_GENERATION);
    ...
    public static final String SYSTEM_PROMPT_CHAPTER_JSON = """
            ...
            全局数据与风格指令，你需要：
            1. 完全遵循IR版本 %s 的结构，严禁输出HTML或Markdown。
            2. 仅使用以下Block类型：%s；其中图表用block.type=widget并填充Chart.js配置。
    ...
    public static final String SYSTEM_PROMPT_CHAPTER_JSON_REPAIR = """
            ...
            请牢记：
            1. 所有chapter必须满足IR版本 %s 约束，仅允许以下block.type：%s；
            2. paragraph.inlines中的marks必须来自以下集合：%s；
            ...
            只返回JSON，不要添加注释或自然语言。
            """.formatted(Schema.IR_VERSION, String.join(", ", Schema.ALLOWED_BLOCK_TYPES), String.join(", ", Schema.ALLOWED_INLINE_MARKS), Schema.CHAPTER_JSON_SCHEMA_TEXT);
    ...
    public static final String SYSTEM_PROMPT_CHAPTER_JSON_RECOVERY = """
            ...
            请遵守：
            1. 章节必须满足IR版本 %s 规范，block.type 仅能使用：%s；
            2. paragraph.inlines中的marks仅可出现：%s，并保留原始文字顺序；
            ...
            请直接返回修复后的JSON。
            """.formatted(Schema.IR_VERSION, String.join(", ", Schema.ALLOWED_BLOCK_TYPES), String.join(", ", Schema.ALLOWED_INLINE_MARKS));
    ...
    public static final String SYSTEM_PROMPT_DOCUMENT_LAYOUT = """
            ...
            **重要的输出格式要求：**
            ...
            """.formatted(DOCUMENT_LAYOUT_OUTPUT_SCHEMA);
    ...
    public static final String SYSTEM_PROMPT_WORD_BUDGET = """
            ...
            **重要的输出格式要求：**
            ...
            4. 确保JSON语法完全正确：
               ...
               - 不要在JSON中添加注释
            5. 所有字符串值使用双引号，数值不使用引号
            """.formatted(WORD_BUDGET_OUTPUT_SCHEMA);
}
